...
% Generates data for Figures 3A-B and 3D-E and writes the data to a file
...
%% Difference learning rate groups

% idx_c = lrSubz < 800;
% mean(lrChangeFull(idx_c,6) - lrChangeFull(idx_c,5))
% idx_p = lrSubz > 799;
% mean(lrChangeFull(idx_p,6) - lrChangeFull(idx_p,5))

for s = 1:nsubj   
    idx = lrSubz == submat(s);    
    diffChange(s) = mean(lrChangeFull(idx,6) - lrChangeFull(idx,5));    
end

change_c = diffChange(unique(lrSubz) < 800);
Mchange_c = mean(change_c);
change_p = diffChange(unique(lrSubz) > 799);
Mchange_p = mean(change_p);

[h,p] = ttest2(change_c,change_p);
